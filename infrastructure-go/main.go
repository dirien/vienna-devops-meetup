...
const (
	clusterName       = "vienna-devops-meetup-eks-cluster"
...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		vpc, err := ec2.NewVpc(ctx, "vienna-devops-meetup-vpc", &ec2.VpcArgs{
			CidrBlock: pulumi.String("172.31.0.0/16"),
		})
		if err != nil {
			return err
		}
		igw, err := ec2.NewInternetGateway(ctx, "vienna-devops-meetup-igw", &ec2.InternetGatewayArgs{
			VpcId: vpc.ID(),
		})
		if err != nil {
			return err
		}
		rt, err := ec2.NewRouteTable(ctx, "vienna-devops-meetup-rt", &ec2.RouteTableArgs{
			VpcId: vpc.ID(),
		...
		for i, az := range availabilityZones {
			publicSubnet, err := ec2.NewSubnet(ctx, fmt.Sprintf("vienna-devops-meetup-subnet-%d", i), &ec2.SubnetArgs{
				VpcId:                       vpc.ID(),
			...
			_, err = ec2.NewRouteTableAssociation(ctx, fmt.Sprintf("vienna-devops-meetup-rt-association-%s", az), &ec2.RouteTableAssociationArgs{
				RouteTableId: rt.ID(),
		...
		albRole, err := iam.NewRole(ctx, "vienna-devops-meetup-alb-role", &iam.RoleArgs{
		...
		albIAMPolicy, err := iam.NewPolicy(ctx, "vienna-devops-meetup-alb-policy", &iam.PolicyArgs{
			Policy: pulumi.String(albPolicyFile),
		}, pulumi.DependsOn([]pulumi.Resource{albRole}))
		if err != nil {
			return err
		}
		_, err = iam.NewRolePolicyAttachment(ctx, "vienna-devops-meetup-alb-role-attachment", &iam.RolePolicyAttachmentArgs{
		...
		provider, err := kubernetes.NewProvider(ctx, "vienna-devops-meetup-k8s-provider", &kubernetes.ProviderArgs{
